defmodule Luex.Shell.Server do
  @moduledoc """
  ...
  defp loop(state, evaluator, evaluator_ref) do
    ...
    input = spawn(fn -> io_get(self_pid, state.prefix, counter) end)
  ...
  def start_evaluator(opts) do
    ...
    evaluator = opts[:evaluator] || :proc_lib.start(Luex.Shell.Evaluator, :init, [:ack, self_pid, self_leader, opts])
    evaluator
  end
end
