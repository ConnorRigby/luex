defmodule Luex.Shell do
  ...
  alias Luex.Shell.Server

  @doc """
  This is the callback invoked by Erlang's shell when someone presses Ctrl+G
  and types `s Elixir.Luex.Shell` or `s sh`.
  """
  def start(opts \\ [], mfa \\ {Luex.Shell, :dont_display_result, []}) do
    spawn(fn ->
...
defmodule :lua do
  @moduledoc """
  This is a shortcut for invoking `Luex.Shell` in the Erlang job
  control menu.  The alternative is to type `:Elixir.Luex.Shell` at
  the `s [shell]` prompt.
  """

  defdelegate start, to: Luex.Shell
end
